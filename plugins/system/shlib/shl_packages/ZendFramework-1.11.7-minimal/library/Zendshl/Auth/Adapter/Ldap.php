 ...
 * @subpackage Zendshl_Auth_Adapter
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Auth_Adapter_Ldap implements Zendshl_Auth_Adapter_Interface
{

    /**
     * The Zendshl_Ldap context.
     *
     * @var Zendshl_Ldap
     */
    ...
    /**
     * The array of arrays of Zendshl_Ldap options passed to the constructor.
     *
     * @var array
     */
     ...
     * Constructor
     *
     * @param  array  $options  An array of arrays of Zendshl_Ldap options
    ...
    /**
     * Returns the array of arrays of Zendshl_Ldap options of this adapter.
     *
     * @return array|null
     */
    ...
    /**
     * Sets the array of arrays of Zendshl_Ldap options to be used by
     * this adapter.
     *
     * @param  array $options The array of arrays of Zendshl_Ldap options
     * @return Zendshl_Auth_Adapter_Ldap Provides a fluent interface
     */
     ...
     *
     * @return Zendshl_Ldap The Zendshl_Ldap object used to authenticate the credentials
     */
    public function getLdap()
    {
        if ($this->_ldap === null) {
             ...
             * @see Zendshl_Ldap
             */
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Ldap.php';
            $this->_ldap = new Zendshl_Ldap();
        }
     ...
     *
     * @param Zendshl_Ldap $ldap An existing Ldap object
     * @return Zendshl_Auth_Adapter_Ldap Provides a fluent interface
     */
    public function setLdap(Zendshl_Ldap $ldap)
    {
        $this->_ldap = $ldap;
    ...
    {
        /**
         * @see Zendshl_Ldap_Exception
         */
        require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Ldap/Exception.php';
        ...
        if (!$username) {
            $code = Zendshl_Auth_Result::FAILURE_IDENTITY_NOT_FOUND;
            $messages[0] = 'A username is required';
            return new Zendshl_Auth_Result($code, '', $messages);
        }
        if (!$password) {
            /* A password is required because some servers will
             * treat an empty password as an anonymous bind.
             */
            $code = Zendshl_Auth_Result::FAILURE_CREDENTIAL_INVALID;
        ...
        foreach ($this->_options as $name => $options) {
            ...
            if (!is_array($options)) {
                ...
                throw new Zendshl_Auth_Adapter_Exception('Adapter options array not an array');
            }
            ...
            try {
                 ...
                 * "password" in the Zendshl_Ldap options is able to retrieve the required
                 * information.
                 */
                $requireRebind = false;
                ...
                $dn = $ldap->getCanonicalAccountName($canonicalName, Zendshl_Ldap::ACCTNAME_FORM_DN);
                ...
                if ($err == Zendshl_Ldap_Exception::LDAP_X_DOMAIN_MISMATCH) {
                     ...
                     */
                    continue;
                } else if ($err == Zendshl_Ldap_Exception::LDAP_NO_SUCH_OBJECT) {
                    $code = Zendshl_Auth_Result::FAILURE_IDENTITY_NOT_FOUND;
                    $messages[0] = "Account not found: $username";
                    $failedAuthorities[$dname] = $zle->getMessage();
                } else if ($err == Zendshl_Ldap_Exception::LDAP_INVALID_CREDENTIALS) {
                    $code = Zendshl_Auth_Result::FAILURE_CREDENTIAL_INVALID;
        ...
        return new Zendshl_Auth_Result($code, $username, $messages);
    }

    /**
     * Sets the LDAP specific options on the Zendshl_Ldap instance
     *
     * @param  Zendshl_Ldap $ldap
     * @param  array $options
     * @return array of auth-adapter specific options
     */
    protected function _prepareOptions(Zendshl_Ldap $ldap, array $options)
    {
        $adapterOptions = array(
            'group'       => null,
            'groupDn'     => $ldap->getBaseDn(),
            'groupScope'  => Zendshl_Ldap::SEARCH_SCOPE_SUB,
            'groupAttr'   => 'cn',
        ...
        foreach ($adapterOptions as $key => $value) {
            if (array_key_exists($key, $options)) {
                ...
                switch ($key) {
                    case 'groupScope':
                        $value = (int)$value;
                        if (in_array($value, array(Zendshl_Ldap::SEARCH_SCOPE_BASE,
                                Zendshl_Ldap::SEARCH_SCOPE_ONE, Zendshl_Ldap::SEARCH_SCOPE_SUB), true)) {
     ...
     * @return string|true
     */
    protected function _checkGroupMembership(Zendshl_Ldap $ldap, $canonicalName, $dn, array $adapterOptions)
    {
        ...
        /**
         * @see Zendshl_Ldap_Filter
         */
        require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Ldap/Filter.php';
        $groupName = Zendshl_Ldap_Filter::equals($adapterOptions['groupAttr'], $adapterOptions['group']);
        $membership = Zendshl_Ldap_Filter::equals($adapterOptions['memberAttr'], $user);
        $group = Zendshl_Ldap_Filter::andFilter($groupName, $membership);
     ...
     *
     * This resembles the feature {@see Zendshl_Auth_Adapter_DbTable::getResultRowObject()}.
     * Closes ZF-6813
     *
