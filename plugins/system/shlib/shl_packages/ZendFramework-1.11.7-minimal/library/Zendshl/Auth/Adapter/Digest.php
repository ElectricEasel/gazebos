 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Auth_Adapter_Digest implements Zendshl_Auth_Adapter_Interface
{
    /**
     ...
     *
     * @param  mixed $filename
     * @return Zendshl_Auth_Adapter_Digest Provides a fluent interface
     */
    ...
    public function authenticate()
    {
        ...
        foreach ($optionsRequired as $optionRequired) {
            if (null === $this->{"_$optionRequired"}) {
                ...
                throw new Zendshl_Auth_Adapter_Exception("Option '$optionRequired' must be set before authentication");
            }
        }
        ...
        if (false === ($fileHandle = @fopen($this->_filename, 'r'))) {
             ...
             */
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Auth/Adapter/Exception.php';
            throw new Zendshl_Auth_Adapter_Exception("Cannot open '$this->_filename' for reading");
        }
        ...
        while ($line = trim(fgets($fileHandle))) {
            if (substr($line, 0, $idLength) === $id) {
                if ($this->_secureStringCompare(substr($line, -32), md5("$this->_username:$this->_realm:$this->_password"))) {
                    $result['code'] = Zendshl_Auth_Result::SUCCESS;
                } else {
                    $result['code'] = Zendshl_Auth_Result::FAILURE_CREDENTIAL_INVALID;
                    $result['messages'][] = 'Password incorrect';
                }
                return new Zendshl_Auth_Result($result['code'], $result['identity'], $result['messages']);
            }
        }

        $result['code'] = Zendshl_Auth_Result::FAILURE_IDENTITY_NOT_FOUND;
