...
/** Zendshl_Loader_Autoloader_Interface */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Autoloader/Interface.php';

/**
 * Resource loader
 *
 * @uses       Zendshl_Loader_Autoloader_Interface
 * @package    Zendshl_Loader
 * @subpackage Autoloader
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Loader_Autoloader_Resource implements Zendshl_Loader_Autoloader_Interface
{
    /**
     ...
     * Constructor
     *
     * @param  array|Zendshl_Config $options Configuration options for resource autoloader
     * @return void
     */
    public function __construct($options)
    {
        if ($options instanceof Zendshl_Config) {
            $options = $options->toArray();
        }
        if (!is_array($options)) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
            throw new Zendshl_Loader_Exception('Options must be passed to resource loader constructor');
        }
        ...
        if ((null === $namespace)
            ...
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
            throw new Zendshl_Loader_Exception('Resource loader requires both a namespace and a base path for initialization');
        }

        if (!empty($namespace)) {
            $namespace .= '_';
        }
        require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Autoloader.php';
        Zendshl_Loader_Autoloader::getInstance()->unshiftAutoloader($this, $namespace);
    }
     ...
     * syntax. Example:
     * <code>
     * $loader = new Zendshl_Loader_Autoloader_Resource(array(
     *     'namespace' => 'Stuff_',
     ...
     * @param  array $args
     * @return mixed
     * @throws Zendshl_Loader_Exception if method not beginning with 'get' or not matching a valid resource type is called
     */
    public function __call($method, $args)
    {
        if ('get' == substr($method, 0, 3)) {
            ...
            if (!$this->hasResourceType($type)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
                throw new Zendshl_Loader_Exception("Invalid resource type $type; cannot load resource");
            }
            if (empty($args)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
                throw new Zendshl_Loader_Exception("Cannot load resources; no resource specified");
            }
        ...
        require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
        throw new Zendshl_Loader_Exception("Method '$method' is not supported");
    }
    ...
    public function getClassPath($class)
    {
        ...
        if (Zendshl_Loader::isReadable($classPath)) {
            return $classPath;
        }
     ...
     *
     * @param  array $options
     * @return Zendshl_Loader_Autoloader_Resource
     */
    ...
    public function addResourceType($type, $path, $namespace = null)
    {
        ...
        if (!isset($this->_resourceTypes[$type])) {
            if (null === $namespace) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
                throw new Zendshl_Loader_Exception('Initial definition of a resource type must include a namespace');
            }
            $namespaceTopLevel = $this->getNamespace();
        ...
        if (!is_string($path)) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
            throw new Zendshl_Loader_Exception('Invalid path specification provided; must be string');
        }
    ...
    public function addResourceTypes(array $types)
    {
        foreach ($types as $type => $spec) {
            if (!is_array($spec)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
                throw new Zendshl_Loader_Exception('addResourceTypes() expects an array of arrays');
            }
            if (!isset($spec['path'])) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
                throw new Zendshl_Loader_Exception('addResourceTypes() expects each array to include a paths element');
            }
            $paths  = $spec['path'];
            $namespace = null;
     ...
     *
     * @see    Zendshl_Loader_Autoloader_Resource::addResourceTypes()
     * @param  array $types
     * @return Zendshl_Loader_Autoloader_Resource
     */
     ...
     * @param  string $type
     * @return object
     * @throws Zendshl_Loader_Exception if resource type not specified or invalid
     */
    public function load($resource, $type = null)
    {
        if (null === $type) {
            ...
            if (empty($type)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
                throw new Zendshl_Loader_Exception('No resource type specified');
            }
        }
        if (!$this->hasResourceType($type)) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Loader/Exception.php';
            throw new Zendshl_Loader_Exception('Invalid resource type specified');
        }
        $namespace = $this->_resourceTypes[$type]['namespace'];
