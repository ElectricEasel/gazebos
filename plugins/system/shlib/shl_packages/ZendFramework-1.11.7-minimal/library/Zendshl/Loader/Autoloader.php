 ...
 *
 * @uses       Zendshl_Loader_Autoloader
 * @package    Zendshl_Loader
 * @subpackage Autoloader
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Loader_Autoloader
{
    /**
     * @var Zendshl_Loader_Autoloader Singleton instance
     */
     ...
     */
    protected $_defaultAutoloader = array('Zendshl_Loader', 'loadClass');
     ...
     *
     * @return Zendshl_Loader_Autoloader
     */
    ...
    public static function autoload($class)
    {
        ...
        foreach ($self->getClassAutoloaders($class) as $autoloader) {
            if ($autoloader instanceof Zendshl_Loader_Autoloader_Interface) {
    ...
    public function setDefaultAutoloader($callback)
    {
        if (!is_callable($callback)) {
            throw new Zendshl_Loader_Exception('Invalid callback specified for default autoloader');
        }
     ...
     *
     * @param  array $autoloaders Array of PHP callbacks (or Zendshl_Loader_Autoloader_Interface implementations) to act as autoloaders
     * @return Zendshl_Loader_Autoloader
     */
    ...
    public function registerNamespace($namespace)
    {
        if (is_string($namespace)) {
            ...
            throw new Zendshl_Loader_Exception('Invalid namespace provided');
        }
    ...
    public function setZfPath($spec, $version = 'latest')
    {
        ...
        if (is_array($spec)) {
            if (!isset($spec['path'])) {
                throw new Zendshl_Loader_Exception('No path specified for ZF');
            }
            $path = $spec['path'];
        ...
        $this->_zfPath = $spec . '/ZendFramework-1.11.7-minimal/library';
     ...
     * @return bool|Zendshl_Loader_Autoloader Returns boolean if no argument is passed, object instance otherwise
     */
     ...
     *
     * @param  object|array|string $callback PHP callback or Zendshl_Loader_Autoloader_Interface implementation
    ...
    protected function _getVersionPath($path, $version)
    {
        ...
        if (empty($availableVersions)) {
            throw new Zendshl_Loader_Exception('No valid ZF installations discovered');
        }
     ...
     * @throws Zendshl_Loader_Exception if version string contains too many dots
     */
    protected function _getVersionType($version)
    {
        ...
        if (3 < $count) {
            throw new Zendshl_Loader_Exception('Invalid version string provided');
        }
        return 'specific';
    }
    ...
    {
        if (!is_dir($path)) {
            throw new Zendshl_Loader_Exception('Invalid ZF path provided');
        }
