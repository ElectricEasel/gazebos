...
/**
 * Zendshl_Server_Reflection_Node
 */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Server/Reflection/Node.php';
...
/**
 * Zendshl_Server_Reflection_Prototype
 */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Server/Reflection/Prototype.php';
 ...
 * method {@link Zendshl_Server_Reflection_Prototype prototypes}.
 *
 * @category   Zend
 * @package    Zendshl_Server
 ...
 */
abstract class Zendshl_Server_Reflection_Function_Abstract
{
    /**
    ...
    public function __construct(Reflector $r, $namespace = null, $argv = array())
    {
        ...
        if ((!$r instanceof ReflectionFunction)
            ...
            throw new Zendshl_Server_Reflection_Exception('Invalid reflection class');
        }
        $this->_reflection = $r;
     ...
     *
     * @param Zendshl_Server_Reflection_Node $parent
     * @param int $level
     * @return void
     */
    protected function _addTree(Zendshl_Server_Reflection_Node $parent, $level = 0)
    {
        ...
        foreach ($this->_sigParams[$level] as $value) {
            $node = new Zendshl_Server_Reflection_Node($value, $parent);
     ...
     * {@link Zendshl_Server_Reflection_Node}s.
     *
     * @return array
     */
    protected function _buildTree()
    {
        ...
        foreach ((array) $this->_return as $value) {
            $node = new Zendshl_Server_Reflection_Node($value);
            $this->_addTree($node);
            $returnTree[] = $node;
        }
    ...
    protected function _buildSignatures($return, $returnDesc, $paramTypes, $paramDesc)
    {
        ...
        foreach ($endPoints as $node) {
            if (!$node instanceof Zendshl_Server_Reflection_Node) {
                continue;
            }
            ...
            do {
                ...
                $node = $node->getParent();
            } while ($node instanceof Zendshl_Server_Reflection_Node);
        ...
        foreach ($signatures as $signature) {
            $return = new Zendshl_Server_Reflection_ReturnValue(array_shift($signature), $this->_returnDesc);
            $tmp    = array();
            foreach ($signature as $key => $type) {
                $param = new Zendshl_Server_Reflection_Parameter($params[$key], $type, (isset($this->_paramDesc[$key]) ? $this->_paramDesc[$key] : null));
                $param->setPosition($key);
                $tmp[] = $param;
            }

            $this->_prototypes[] = new Zendshl_Server_Reflection_Prototype($return, $tmp);
        }
    }
    ...
    protected function _reflect()
    {
        ...
        if (count($paramTypesTmp) != $paramCount) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Server/Reflection/Exception.php';
            throw new Zendshl_Server_Reflection_Exception(
    ...
    public function __call($method, $args)
    {
        ...
        require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Server/Reflection/Exception.php';
        throw new Zendshl_Server_Reflection_Exception('Invalid reflection method ("' .$method. '")');
    }
    ...
    public function setNamespace($namespace)
    {
        ...
        if (!is_string($namespace) || !preg_match('/[a-z0-9_\.]+/i', $namespace)) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Server/Reflection/Exception.php';
            throw new Zendshl_Server_Reflection_Exception('Invalid namespace');
        }
    ...
    public function setDescription($string)
    {
        if (!is_string($string)) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Server/Reflection/Exception.php';
            throw new Zendshl_Server_Reflection_Exception('Invalid description');
        }
     ...
     * {@link Zendshl_Server_Reflection_Prototype Zendshl_Server_Reflection_Prototypes}
     *
     * @return array
     */
