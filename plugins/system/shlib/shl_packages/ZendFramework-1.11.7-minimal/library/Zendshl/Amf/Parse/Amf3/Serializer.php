...
/** Zendshl_Amf_Constants */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Constants.php';


/** Zendshl_Amf_Parse_Serializer */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Parse/Serializer.php';

/** Zendshl_Amf_Parse_TypeLoader */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Amf_Parse_Amf3_Serializer extends Zendshl_Amf_Parse_Serializer
{
    /**
    ...
    public function writeTypeMarker(&$data, $markerType = null, $dataByVal = false)
    {
        ...
        if (null !== $markerType) {
            ...
            switch ($markerType) {
                case Zendshl_Amf_Constants::AMF3_NULL:
                    break;
                case Zendshl_Amf_Constants::AMF3_BOOLEAN_FALSE:
                    break;
                case Zendshl_Amf_Constants::AMF3_BOOLEAN_TRUE:
                    break;
                case Zendshl_Amf_Constants::AMF3_INTEGER:
                    $this->writeInteger($data);
                    break;
                case Zendshl_Amf_Constants::AMF3_NUMBER:
                    $this->_stream->writeDouble($data);
                    break;
                case Zendshl_Amf_Constants::AMF3_STRING:
                    $this->writeString($data);
                    break;
                case Zendshl_Amf_Constants::AMF3_DATE:
                    $this->writeDate($data);
                    break;
                case Zendshl_Amf_Constants::AMF3_ARRAY:
                    $this->writeArray($data);
                    break;
                case Zendshl_Amf_Constants::AMF3_OBJECT:
                    $this->writeObject($data);
                    break;
                case Zendshl_Amf_Constants::AMF3_BYTEARRAY:
                    $this->writeByteArray($data);
                    break;
                case Zendshl_Amf_Constants::AMF3_XMLSTRING;
                    $this->writeXml($data);
                    break;
                default:
                    require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                    throw new Zendshl_Amf_Exception('Unknown Type Marker: ' . $markerType);
            }
        } else {
            // Detect Type Marker
            if (is_resource($data)) {
                $data = Zendshl_Amf_Parse_TypeLoader::handleResource($data);
            }
            switch (true) {
                case (null === $data):
                    $markerType = Zendshl_Amf_Constants::AMF3_NULL;
                    break;
                case (is_bool($data)):
                    if ($data){
                        $markerType = Zendshl_Amf_Constants::AMF3_BOOLEAN_TRUE;
                    } else {
                        $markerType = Zendshl_Amf_Constants::AMF3_BOOLEAN_FALSE;
                    }
                    break;
                case (is_int($data)):
                    if (($data > 0xFFFFFFF) || ($data < -268435456)) {
                        $markerType = Zendshl_Amf_Constants::AMF3_NUMBER;
                    } else {
                        $markerType = Zendshl_Amf_Constants::AMF3_INTEGER;
                    }
                    break;
                case (is_float($data)):
                    $markerType = Zendshl_Amf_Constants::AMF3_NUMBER;
                    break;
                case (is_string($data)):
                    $markerType = Zendshl_Amf_Constants::AMF3_STRING;
                    break;
                case (is_array($data)):
                    $markerType = Zendshl_Amf_Constants::AMF3_ARRAY;
                    break;
                case (is_object($data)):
                    // Handle object types.
                    if (($data instanceof DateTime) || ($data instanceof Zendshl_Date)) {
                        $markerType = Zendshl_Amf_Constants::AMF3_DATE;
                    } else if ($data instanceof Zendshl_Amf_Value_ByteArray) {
                        $markerType = Zendshl_Amf_Constants::AMF3_BYTEARRAY;
                    } else if (($data instanceof DOMDocument) || ($data instanceof SimpleXMLElement)) {
                        $markerType = Zendshl_Amf_Constants::AMF3_XMLSTRING;
                    } else {
                        $markerType = Zendshl_Amf_Constants::AMF3_OBJECT;
                    }
                    break;
                default:
                    require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                    throw new Zendshl_Amf_Exception('Unsupported data type: ' . gettype($data));
            }
     ...
     * @return Zendshl_Amf_Parse_Amf3_Serializer
     */
     ...
     *
     * @param  string|Zendshl_Amf_Value_ByteArray  $data
     * @return Zendshl_Amf_Parse_Amf3_Serializer
     */
    public function writeByteArray(&$data)
    {
        ...
        if (is_string($data)) {
            ...
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
            throw new Zendshl_Amf_Exception('Invalid ByteArray specified; must be a string or Zendshl_Amf_Value_ByteArray');
        }
    ...
    public function writeXml($xml)
    {
        ...
        if(is_string($xml)) {
            ...
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
            throw new Zendshl_Amf_Exception('Invalid xml specified; must be a DOMDocument or SimpleXMLElement');
        }
    ...
    /**
     * Convert DateTime/Zendshl_Date to AMF date
     *
     * @param  DateTime|Zendshl_Date $date
     * @return Zendshl_Amf_Parse_Amf3_Serializer
     */
    public function writeDate($date)
    {
        ...
        } elseif ($date instanceof Zendshl_Date) {
            $dateString = $date->toString('U') * 1000;
        } else {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
            throw new Zendshl_Amf_Exception('Invalid date specified; must be a string DateTime or Zendshl_Date object');
        }
    ...
    public function writeObject($object)
    {
        ...
        switch (true) {
            ...
            case ($className = Zendshl_Amf_Parse_TypeLoader::getMappedClassName(get_class($object))):
                break;
        ...
        if(array_key_exists($className, $this->_referenceDefinitions)){
            ...
            if($className == ''){
                ...
                $encoding = Zendshl_Amf_Constants::ET_DYNAMIC;
            } else {
                $encoding = Zendshl_Amf_Constants::ET_PROPLIST;
            ...
            $traitsInfo = Zendshl_Amf_Constants::AMF3_OBJECT_ENCODING;
        ...
        try {
            switch($encoding) {
                ...
                case Zendshl_Amf_Constants::ET_DYNAMIC:
                    ...
                    $this->writeString($this->_strEmpty);
                    break;
                case Zendshl_Amf_Constants::ET_EXTERNAL:
                    require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                    throw new Zendshl_Amf_Exception('External Object Encoding not implemented');
                    break;
                default:
                    require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                    throw new Zendshl_Amf_Exception('Unknown Object Encoding type: ' . $encoding);
            }
        } catch (Exception $e) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
            throw new Zendshl_Amf_Exception('Unable to writeObject output: ' . $e->getMessage(), 0, $e);
        }
