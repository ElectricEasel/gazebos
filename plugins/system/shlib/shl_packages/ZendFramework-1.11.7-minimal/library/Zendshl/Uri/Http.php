...
/**
 * @see Zendshl_Validate_Hostname
 */
//require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Validate/Hostname.php';
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Uri_Http extends Zendshl_Uri
{
  /**
   ...
   * @throws Zendshl_Uri_Exception When the URI is not valid
   */
  protected function __construct($scheme, $schemeSpecific = '')
  {
    ...
    if ($this->valid() === false) {
      ...
      throw new Zendshl_Uri_Exception('Invalid URI supplied');
    }
  }

  /**
   * Creates a Zendshl_Uri_Http from the given string
   *
  ...
  public static function fromString($uri)
  {
    if (is_string($uri) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('$uri is not a string');
    }
    ...
    if (in_array($scheme, array('http', 'https')) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("Invalid scheme: '$scheme'");
    }

    $schemeHandler = new Zendshl_Uri_Http($scheme, $schemeSpecific);
   ...
   * @throws Zendshl_Uri_Exception When scheme-specific decoposition fails
   * @throws Zendshl_Uri_Exception When authority decomposition fails
   * @return void
   */
  protected function _parseUri($schemeSpecific)
  {
    ...
    if ($status === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('Internal error: scheme-specific decomposition failed');
    }
    ...
    if ($status === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('Internal error: authority decomposition failed');
    }
   ...
   *
   * @throws Zendshl_Uri_Exception When one or more parts of the URI are invalid
   * @return string
   */
  public function getUri()
  {
    if ($this->valid() === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('One or more parts of the URI are invalid');
    }
   ...
   *
   *@param boolean URI validation is disabled if false. Added as validation just kills server on some long URI - shumisha - 2012-03-05
   * @return boolean
   */
  public function valid( $validationEnabled = false)
  {
    ...
    and $this->validateQuery( null, $validationEnabled)
   ...
   * @throws Zendshl_Uri_Exception When username validation fails
   * @return boolean
  ...
  public function validateUsername($username = null)
  {
    ...
    if ($status === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('Internal error: username validation failed');
    }
   ...
   *
   * @param  string $username The HTTP username
   * @throws Zendshl_Uri_Exception When $username is not a valid HTTP username
   * @return string
   */
  public function setUsername($username)
  {
    if ($this->validateUsername($username) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("Username \"$username\" is not a valid HTTP username");
    }
   ...
   * @throws Zendshl_Uri_Exception When password validation fails
   * @return boolean
   * @link   http://www.faqs.org/rfcs/rfc2396.html
   */
  public function validatePassword($password = null)
  {
    ...
    if ($status === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('Internal error: password validation failed.');
    }
   ...
   *
   * @param  string $password The HTTP password
   * @throws Zendshl_Uri_Exception When $password is not a valid HTTP password
   * @return string
   */
  public function setPassword($password)
  {
    if ($this->validatePassword($password) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("Password \"$password\" is not a valid HTTP password.");
    }
  ...
  public function validateHost($host = null)
  {
    ...
    // Check the host against the allowed values; delegated to Zendshl_Filter.
    $validate = new Zendshl_Validate_Hostname(Zendshl_Validate_Hostname::ALLOW_ALL);
   ...
   *
   * @param  string $host The HTTP host
   * @throws Zendshl_Uri_Exception When $host is nota valid HTTP host
   * @return string
   */
  public function setHost($host)
  {
    if ($this->validateHost($host) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("Host \"$host\" is not a valid HTTP host");
    }
   ...
   *
   * @param  string $port The HTTP port
   * @throws Zendshl_Uri_Exception When $port is not a valid HTTP port
   * @return string
   */
  public function setPort($port)
  {
    if ($this->validatePort($port) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("Port \"$port\" is not a valid HTTP port.");
    }
   ...
   * @throws Zendshl_Uri_Exception When path validation fails
   * @return boolean
   */
  public function validatePath($path = null)
  {
    ...
    if ($status === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('Internal error: path validation failed');
    }
   ...
   *
   * @param  string $path The HTTP path
   * @throws Zendshl_Uri_Exception When $path is not a valid HTTP path
   * @return string
   */
  public function setPath($path)
  {
    if ($this->validatePath($path) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("Path \"$path\" is not a valid HTTP path");
    }
   ...
   * @param boolean URI validation is disabled if false. Added as validation just kills server on some long URI - shumisha - 2012-03-05
   * @throws Zendshl_Uri_Exception When query validation fails
   * @return boolean
   * @link   http://www.faqs.org/rfcs/rfc2396.html
   */
  public function validateQuery($query = null, $validationEnabled = false)
  {
    // shumisha 2012-03-05 allow disabling query validation
    ...
    if ($status === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('Internal error: query validation failed');
    }
   ...
   * @param boolean URI validation is disabled if false. Added as validation just kills server on some long URI - shumisha - 2012-03-05
   * @throws Zendshl_Uri_Exception When $query is not a valid query string
   * @return string              Old query string
   */
  public function setQuery($query, $validationEnabled = false)
  {
    ...
    if (is_array($query) === true) {
      ...
      $query = (string) $query;
      if ($this->validateQuery($query, $validationEnabled) === false) {
    ...
    if ($this->validateQuery($query, $validationEnabled) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("'$query' is not a valid query string");
    }
   ...
   * @throws Zendshl_Uri_Exception When fragment validation fails
   * @return boolean
   * @link   http://www.faqs.org/rfcs/rfc2396.html
   */
  public function validateFragment($fragment = null)
  {
    ...
    if ($status === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception('Internal error: fragment validation failed');
    }
   ...
   * @throws Zendshl_Uri_Exception When $fragment is not a valid HTTP fragment
   * @return string
   */
  public function setFragment($fragment)
  {
    if ($this->validateFragment($fragment) === false) {
      require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Uri/Exception.php';
      throw new Zendshl_Uri_Exception("Fragment \"$fragment\" is not a valid HTTP fragment");
    }
