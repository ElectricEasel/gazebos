...
<div class="product-top">
	...
	<table id="product-size-chart">
		...
		foreach (array_chunk($this->item->sizes, 5) as $chunk)
		{
			...
			for ($i=0;$i<5;$i++)
			{
				...
				if (!empty($s->id))
				{
					$link = JRoute::_('index.php?option=com_gazebos&view=size&id=' . $s->id);
					echo '<td><a href="' . $link . '">' . $s->size . '</a></td>';
				}
				else
				{
					echo '<td></td>';
	...
	<p style="font-size:14px;"><span style="color:#463228;font-style:italic;font-weight:bold;">Looking for a different size?</span> <span style="font-size:13px;color:#897c6b;font-weight:bold">Our products can be built to fit your specific needs.</span></p>
</div>
