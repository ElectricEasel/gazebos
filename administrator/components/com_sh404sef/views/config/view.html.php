...
class Sh404sefViewConfig extends ShlMvcView_Base {
  ...
  public function display( $tpl = null) {
    ...
    JHtml::script( Sh404sefHelperGeneral::getComponentUrl() .  '/assets/js/config.js');
  ...
  private function _pushConfigDataDefault() {
    ...
    $lists['activeLanguages'][] = $shPageInfo->currentLanguageTag;  // put default in first place
    ...
    foreach ($activeLanguages as $language) {
      ...
      if ($currentLang != $shPageInfo->currentLanguageTag) $lists['activeLanguages'][] = $currentLang;
    ...
    // 3.4
    $lists['enableMultiLingualSupport'] =  JHTML::_('select.booleanlist', 'enableMultiLingualSupport', $std_opt, $sefConfig->enableMultiLingualSupport );

    // 3.6

    $urlCacheHandlerOpt[] = JHTML::_('select.option', 'File', JText::_('COM_SH404SEF_URL_CACHE_HANDLER_FILE') );
    $urlCacheHandlerOpt[] = JHTML::_('select.option', 'Sharedmemory', JText::_('COM_SH404SEF_URL_CACHE_HANDLER_SHARED_MEMORY') );

    $lists['UrlCacheHandler'] = JHTML::_('select.genericlist', $urlCacheHandlerOpt, 'UrlCacheHandler',
        "class=\"inputbox\" size=\"1\"" , 'value', 'text',  $sefConfig->UrlCacheHandler);
    $lists['displayUrlCacheStats'] =  JHTML::_('select.booleanlist', 'displayUrlCacheStats', $std_opt, $sefConfig->displayUrlCacheStats );
    ...
    $lists['shlibPluginId'] = empty( $pluginId) ? 0 : $pluginId;

    if(!empty( $lists['shlibPluginId'])) {
      $link = 'index.php?option=com_plugins&task=plugin.edit&extension_id=' . $lists['shlibPluginId'];
      $lists['linkToCachePluginParams'] = '&nbsp;&nbsp;<a href="'.$link.'" target="_blank">' . JText::_('COM_SH404SEF_CONFIGURE_SHLIB_PLUGIN') . '</a>';
    } else {
      $lists['linkToCachePluginParams'] = '';
    }
  ...
  private function _pushConfigDataAnalytics() {
    ...
    $userGroupsOptions = JHtml::_('user.groups', $includeSuperAdmin = true);
    if(is_null($sefConfig->analyticsUserGroups)) {
    	// not set yet: all groups are added
    	$sefConfig->analyticsUserGroups = array();
    	foreach($userGroupsOptions as $groupOption) {
    		$sefConfig->analyticsUserGroups[] = $groupOption->value;
    	}
    }
    $lists['analyticsUserGroups'] = JHTML::_('select.genericlist', $userGroupsOptions, 'analyticsUserGroups[]',
    				'class="inputbox" size="10" multiple="multiple"' ,
    				'value', 'text', $sefConfig->analyticsUserGroups);
  ...
  private function _pushConfigDataExt() {
    ...
    $category = array();
    $category[] = JHTML::_('select.option', '', JText::_('COM_SH404SEF_INSERT_NUMERICAL_ID_ALL_CAT') );
  ...
  private function _pushConfigDataSeo() {
    ...
    */
    ...
    $contentTitleIncludeCat[] = JHTML::_('select.option', shSEFConfig::CAT_NONE, JText::_('COM_SH404SEF_CAT_NONE') );
    ...
    $lists['mobile_switch_enabled'] = JHTML::_('select.booleanlist','mobile_switch_enabled', $std_opt, $enabled );
    $lists['mobile_template'] = $params->get('mobile_template');

    // V 3.4
    $lists['insertPaginationTags'] =  JHTML::_('select.booleanlist','insertPaginationTags',
        $std_opt, $sefConfig->insertPaginationTags );
  ...
  private function _pushConfigDataSocial_seo() {
    ...
    $lists['og_type_select'] = Sh404sefHelperOgp::buildOpenGraphTypesList($sefConfig->ogType, 'ogType');
  ...
  private function _pushConfigDataErrorDocs() {
    ...
    $sh404sefContentCatId = Sh404sefHelperCategories::getSh404sefContentCat();

    try {

      // read current content of 404 page in default language
      $article = ShlDbHelper::selectResult('#__content', array( 'id', 'introtext'), array( 'title' => '__404__' , 'catid' => Sh404sefHelperCategories::getSh404sefContentCat()->id, 'language' => '*'));
