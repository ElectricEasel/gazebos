...
class Sh404sefModelAliases extends Sh404sefClassBaselistModel {
      ...
      if (!empty( $aliasList[0])) {
        ...
        foreach($aliasList as $alias) {
          ...
          if (!empty($alias)) {
            ...
            try {
              $count = ShlDbHelper::count( '#__sh404sef_aliases', 'id', 'oldurl = ? and newurl <> ?', array($alias, ''));
  ...
  public function getUrlByAliasId( $aliasId) {
    ...
    $query  = 'select r.* from ' . $this->_db->quoteName( '#__sh404sef_urls') . ' as r'
    . ' left join ' . $this->_db->quoteName( '#__sh404sef_aliases') . ' as a'
    . ' on a.' . $this->_db->quoteName( 'newurl') . ' = r.' . $this->_db->quoteName( 'newurl')
    . ' where a.' . $this->_db->quoteName( 'id') . ' = ' . $this->_db->Quote( $aliasId)
    . ' order by ' . $this->_db->quoteName( 'rank');
  ...
  private function _getPurgeQuerySelected() {
    ...
    // start delete query
    $deleteQuery = 'delete from ' . $this->_db->quoteName( $this->_getTableName())
