...
/**
 * @see Zendshl_Crypt_Rsa_Key_Private
 */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Crypt/Rsa/Key/Private.php';

/**
 * @see Zendshl_Crypt_Rsa_Key_Public
 */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Crypt/Rsa/Key/Public.php';
...
class Zendshl_Crypt_Rsa
{
     ...
     *
     * @param array $options
     * @throws Zendshl_Crypt_Rsa_Exception
     */
    public function __construct(array $options = null)
    {
        if (!extension_loaded('openssl')) {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Crypt/Rsa/Exception.php';
            throw new Zendshl_Crypt_Rsa_Exception('Zendshl_Crypt_Rsa requires openssl extention to be loaded.');
        }
    ...
    /**
     * @param string $data
     * @param Zendshl_Crypt_Rsa_Key_Private $privateKey
     * @param string $format
     * @return string
     */
    public function sign($data, Zendshl_Crypt_Rsa_Key_Private $privateKey = null, $format = null)
    {
        $signature = '';
        if (isset($privateKey)) {
    ...
    /**
     * @param string $data
     * @param Zendshl_Crypt_Rsa_Key $key
     * @param string $format
     * @return string
     */
    public function encrypt($data, Zendshl_Crypt_Rsa_Key $key, $format = null)
    {
        $encrypted = '';
        $function = 'openssl_public_encrypt';
        if ($key instanceof Zendshl_Crypt_Rsa_Key_Private) {
     ...
     * @return string
     */
    public function decrypt($data, Zendshl_Crypt_Rsa_Key $key, $format = null)
    {
        $decrypted = '';
        if ($format == self::BASE64) {
        ...
        if ($key instanceof Zendshl_Crypt_Rsa_Key_Public) {
    ...
    public function generateKeys(array $configargs = null)
    {
        ...
        $privateKey = new Zendshl_Crypt_Rsa_Key_Private($private, $passPhrase);
        $details = openssl_pkey_get_details($resource);
        $publicKey = new Zendshl_Crypt_Rsa_Key_Public($details['key']);
    ...
    public function setPemString($value)
    {
        ...
        try {
            $this->_privateKey = new Zendshl_Crypt_Rsa_Key_Private($this->_pemString, $this->_passPhrase);
            $this->_publicKey = $this->_privateKey->getPublicKey();
        } catch (Zendshl_Crypt_Exception $e) {
            $this->_privateKey = null;
            $this->_publicKey = new Zendshl_Crypt_Rsa_Key_Public($this->_pemString);
        }
    }
    ...
    public function setCertificateString($value)
    {
        ...
        $this->_publicKey = new Zendshl_Crypt_Rsa_Key_Public($this->_certificateString, $this->_passPhrase);
    }
