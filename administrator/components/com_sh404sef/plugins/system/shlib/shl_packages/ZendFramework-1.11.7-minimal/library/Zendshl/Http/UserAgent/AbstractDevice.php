...
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Http/UserAgent/Device.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Zendshl_Http_UserAgent_AbstractDevice
    implements Zendshl_Http_UserAgent_Device
{
    /**
     * Browser signature
     *
     ...
     * @return Zendshl_Http_UserAgent_AbstractDevice
     */
    ...
    protected function _loadFeaturesAdapter()
    {
        ...
        if (empty($config['classname'])) {
            ...
            throw new Zendshl_Http_UserAgent_Exception('The ' . $this->getType() . ' features adapter must have a "classname" config parameter defined');
        }
        ...
        if (!class_exists($className)) {
            if (isset($config['path'])) {
                ...
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Http/UserAgent/Exception.php';
                throw new Zendshl_Http_UserAgent_Exception('The ' . $this->getType() . ' features adapter must have a "path" config parameter defined');
            }

            if (false === include_once ($path)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Http/UserAgent/Exception.php';
                throw new Zendshl_Http_UserAgent_Exception('The ' . $this->getType() . ' features adapter path that does not exist');
            }
        }
