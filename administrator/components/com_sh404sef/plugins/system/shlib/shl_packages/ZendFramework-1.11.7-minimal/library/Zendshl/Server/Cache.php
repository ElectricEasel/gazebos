...
/**
 * Zendshl_Server_Cache: cache server definitions
 *
 * @category   Zend
 * @package    Zendshl_Server
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
...
class Zendshl_Server_Cache
{
     ...
     * @param  Zendshl_Server_Interface $server
     * @return bool
     */
    public static function save($filename, Zendshl_Server_Interface $server)
    {
        ...
        if ($methods instanceof Zendshl_Server_Definition) {
            $definition = new Zendshl_Server_Definition();
     ...
     *
     * <code>
     * if (!Zendshl_Server_Cache::get($filename, $server)) {
     ...
     *
     *     Zendshl_Server_Cache::save($filename, $server);
     * }
     *
     ...
     * @return bool
     */
    public static function get($filename, Zendshl_Server_Interface $server)
    {
        if (!is_string($filename)
