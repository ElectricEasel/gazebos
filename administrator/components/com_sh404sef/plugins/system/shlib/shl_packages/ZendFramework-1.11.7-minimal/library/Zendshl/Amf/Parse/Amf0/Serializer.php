...
/** @see Zendshl_Amf_Parse_Serializer */
 ...
 *
 * @uses       Zendshl_Amf_Parse_Serializer
 * @package    Zendshl_Amf
 * @subpackage Parse_Amf0
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Amf_Parse_Amf0_Serializer extends Zendshl_Amf_Parse_Serializer
{
    /**
     ...
     * @return Zendshl_Amf_Parse_Amf0_Serializer
     * @throws Zendshl_Amf_Exception for unrecognized types or data
     */
    public function writeTypeMarker(&$data, $markerType = null, $dataByVal = false)
    {
        ...
        if (null !== $markerType) {
            ...
            if (!$this->writeObjectReference($data, $markerType)) {
                ...
                switch($markerType) {
                    case Zendshl_Amf_Constants::AMF0_NUMBER:
                        $this->_stream->writeDouble($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_BOOLEAN:
                        $this->_stream->writeByte($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_STRING:
                        $this->_stream->writeUTF($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_OBJECT:
                        $this->writeObject($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_NULL:
                        break;
                    case Zendshl_Amf_Constants::AMF0_REFERENCE:
                        $this->_stream->writeInt($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_MIXEDARRAY:
                        // Write length of numeric keys as zero.
                        $this->_stream->writeLong(0);
                        $this->writeObject($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_ARRAY:
                        $this->writeArray($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_DATE:
                        $this->writeDate($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_LONGSTRING:
                        $this->_stream->writeLongUTF($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_TYPEDOBJECT:
                        $this->writeTypedObject($data);
                        break;
                    case Zendshl_Amf_Constants::AMF0_AMF3:
            ...
            switch (true) {
                case (is_int($data) || is_float($data)):
                    $markerType = Zendshl_Amf_Constants::AMF0_NUMBER;
                    break;
                case (is_bool($data)):
                    $markerType = Zendshl_Amf_Constants::AMF0_BOOLEAN;
                    break;
                case (is_string($data) && (strlen($data) > 65536)):
                    $markerType = Zendshl_Amf_Constants::AMF0_LONGSTRING;
                    break;
                case (is_string($data)):
                    $markerType = Zendshl_Amf_Constants::AMF0_STRING;
                    break;
                case (is_object($data)):
                    if (($data instanceof DateTime) || ($data instanceof Zendshl_Date)) {
                        $markerType = Zendshl_Amf_Constants::AMF0_DATE;
                    } else {

                        if($className = $this->getClassName($data)){
                            //Object is a Typed object set classname
                            $markerType = Zendshl_Amf_Constants::AMF0_TYPEDOBJECT;
                            $this->_className = $className;
                        } else {
                            // Object is a generic classname
                            $markerType = Zendshl_Amf_Constants::AMF0_OBJECT;
                        }
                        break;
                    }
                    break;
                case (null === $data):
                    $markerType = Zendshl_Amf_Constants::AMF0_NULL;
                    break;
                case (is_array($data)):
                    ...
                    foreach (array_keys($data) as $key) {
                        ...
                        if (!is_numeric($key) || intval($key) != $key) {
                            ...
                            // check if it is a sparse indexed array
                             ...
                             $markerType = Zendshl_Amf_Constants::AMF0_MIXEDARRAY;
                             break;
                         }
                         $i++;
                    }
                    ...
                    if(!$markerType){
                        $markerType = Zendshl_Amf_Constants::AMF0_ARRAY;
                        break;
                    }
                    break;
                default:
    ...
    protected function writeObjectReference(&$object, $markerType, $objectByVal = false)
    {
        ...
        if ($markerType == Zendshl_Amf_Constants::AMF0_OBJECT
            || $markerType == Zendshl_Amf_Constants::AMF0_MIXEDARRAY
            || $markerType == Zendshl_Amf_Constants::AMF0_ARRAY
            || $markerType == Zendshl_Amf_Constants::AMF0_TYPEDOBJECT
        ) {
            ...
            if($ref !== false){
                $this->writeTypeMarker($ref,Zendshl_Amf_Constants::AMF0_REFERENCE);
                return true;
            }
    ...
    public function writeObject($object)
    {
        ...
        $this->_stream->writeByte(Zendshl_Amf_Constants::AMF0_OBJECTTERM);
        return $this;
    }
     ...
     *
     * @param  DateTime|Zendshl_Date $data
     * @return Zendshl_Amf_Parse_Amf0_Serializer
     */
    public function writeDate($data)
    {
        ...
        } elseif ($data instanceof Zendshl_Date) {
            $dateString = $data->toString('U');
        } else {
            require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
            throw new Zendshl_Amf_Exception('Invalid date specified; must be a DateTime or Zendshl_Date object');
        }
        $dateString *= 1000;
    ...
    {
        require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Parse/Amf3/Serializer.php';
        $serializer = new Zendshl_Amf_Parse_Amf3_Serializer($this->_stream);
    ...
    protected function getClassName($object)
    {
        ...
        switch (true) {
            ...
            case Zendshl_Amf_Parse_TypeLoader::getMappedClassName(get_class($object)):
                $className = Zendshl_Amf_Parse_TypeLoader::getMappedClassName(get_class($object));
                break;
