...
/** Zendshl_Amf_Parse_Deserializer */
require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Parse/Deserializer.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zendshl_Amf_Parse_Amf3_Deserializer extends Zendshl_Amf_Parse_Deserializer
{
    /**
     ...
     * @throws Zendshl_Amf_Exception for unidentified marker type
     */
    public function readTypeMarker($typeMarker = null)
    {
        ...
        switch($typeMarker) {
            case Zendshl_Amf_Constants::AMF3_UNDEFINED:
                 return null;
            ...
            case Zendshl_Amf_Constants::AMF3_XML:
            ...
            default:
                ...
                throw new Zendshl_Amf_Exception('Unsupported type marker: ' . $typeMarker);
        }
    }
    ...
    public function readString()
    {
        ...
        if (($stringReference & 0x01) == 0) {
            ...
            if ($stringReference >= count($this->_referenceStrings)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                throw new Zendshl_Amf_Exception('Undefined string reference: ' . $stringReference);
            }
    ...
    public function readDate()
    {
        ...
        if (($dateReference & 0x01) == 0) {
            ...
            if ($dateReference>=count($this->_referenceObjects)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                throw new Zendshl_Amf_Exception('Undefined date reference: ' . $dateReference);
            }
        ...
        require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Date.php';
        $dateTime  = new Zendshl_Date((int) $timestamp);
    ...
    public function readArray()
    {
        ...
        if (($arrayReference & 0x01)==0){
            ...
            if ($arrayReference>=count($this->_referenceObjects)) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                throw new Zendshl_Amf_Exception('Unknow array reference: ' . $arrayReference);
            }
     ...
     *
     * @todo   Rather than using an array of traitsInfo create Zendshl_Amf_Value_TraitsInfo
     * @return object|array
     */
    public function readObject()
    {
        ...
        if ($storedObject) {
            ...
            if (!isset($this->_referenceObjects[$ref])) {
                require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                throw new Zendshl_Amf_Exception('Unknown Object reference: ' . $ref);
            }
            ...
            if ($storedClass) {
                ...
                if (!isset($this->_referenceDefinitions[$ref])) {
                    require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                    throw new Zendshl_Amf_Exception('Unknows Definition reference: '. $ref);
                }
                ...
                // Create a traits table. Zendshl_Amf_Value_TraitsInfo would be ideal
            ...
            if (!$className) {
                ...
                if ($loader = Zendshl_Amf_Parse_TypeLoader::loadType($className)) {
                    $returnObject = new $loader();
                } else {
                    //user defined typed object
                    require_once SHLIB_PATH_TO_ZEND . 'Zendshl/Amf/Exception.php';
                    throw new Zendshl_Amf_Exception('Typed object not found: '. $className . ' ');
                }
            }
       ...
       if ($returnObject instanceof Zendshl_Amf_Value_Messaging_ArrayCollection) {
