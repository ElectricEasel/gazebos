...
class ShlDbClass extends ShlSystem_Abstractdecorator {
  ...
  public $queryCacheTableExclusionList = array( '#__schemas','#__session', '#__update_categories', '#__update_sites', '#__update_sites_extensions', '#__updates');
   ...
   */
  public function shlSetSelectQuery( $table, $aColList = array( '*'), $mWhere = '', $aWhereData = array(), $orderBy = array(), $offset = 0, $lines = 0) {
  ...
  protected function _shlBuildWhereClause( $mWhere = '', $aWhereData = array()) {
    ...
    if (is_string( $mWhere)) {
      ...
      if ($holderCount = 0 || $holderCount != count ( $aWhereData)) {
        ...
        throw new ShlDbException(  __METHOD__ . ': ' . 'Internal error: trying to build invalid db query where clause [ '. serialize( $mWhere). ' ] [ ' . serialize( $aWhereData) . ' ]', 500);
      } else {
    ...
    if(is_string( $orderBy)) {
      ...
      foreach( $orderBy as $key => $value) {
        if(is_int( $key)) {
          ...
          $clause .= ', ' . $this->quoteName( $key)
          .(empty( $value) ? '' : $this->escape( $value));
        }
      }
  ...
  protected function _shlQueryCacheStore( $value, $prefix) {
    ...
    if($this->_shlCanCache()) {
      try {
        ...
        ShlSystem_Log::error( 'shlib', __METHOD__ . ': error storing data in query cache for instance %s, disabling cache for this instance (%s)', $this->instanceName, $e->getMessage());
      }
    }
  ...
  protected function _shlQueryCacheRead( $prefix) {
    ...
    if($this->_shlCanCache()) {
      try {
        ...
        // there was an error: log that and disable cache for this instance
        $this->queryCacheEnabled = false;
        ShlSystem_Log::error( 'shlib', __METHOD__ . ': error reading data in query cache for instance %s, disabling cache for this instance (%s)', $this->instanceName, $e->getMessage());
      }
    }
  ...
  protected function _shlCanCache() {
    ...
    if(strpos( $sql, 'publish_up <=') !== false
        || strpos( $sql, '>= a.publish_up') !== false
        || strpos( $sql, 'publish_down >=') !== false
