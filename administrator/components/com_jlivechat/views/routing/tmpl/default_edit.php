...
<form action="" method="post" name="adminForm" id="adminForm" onsubmit="return selectAllOperators();">
    <?php echo JText::_('ROUTE_NAME_LBL'); ?> <input type="text" name="route_name" value="<?php echo JRequest::getVar('route_name', $this->route['route_name'], 'method'); ?>" size="35" maxlength="150" />
    <br />
    <br />
	...
	<tbody>
	    <?php
	    if(count($this->route['route_rules']) > 0)
	    {
		foreach($this->route['route_rules'] as $ruleArray)
		{
	    ?>
	    <tr <?php if($ruleArray['rule_type'] == '*') { ?>class="place-holder"<?php } ?>>
		<td>
		    <?php
		    if($ruleArray['rule_type'] == '*')
		    {
			echo JText::_('Select all traffic');
		    }
		    else
		    {
			echo $ruleArray['rule_display_name'];
		    }
		    ?>
		</td>
		<td>
		    <?php echo $ruleArray['rule_value']; ?>
		</td>
		<td>
		    <a href="javascript: void(0);" onClick="deleteThisFilter(this);">Delete</a>
		    <input type="hidden" name="source[]" value="<?php echo $ruleArray['rule_type'].':'.$ruleArray['rule_value']; ?>" />
		</td>
	    </tr>
	    <?php
		}
		...
		<td>
		    <?php echo JText::_('Select all traffic'); ?>
		</td>
		<td colspan="2">
		    *:*
	...
	<tr>
		...
		<select style="margin-left: 30px;" id="route_action" name="route_action" onchange="changeAction();">
		    <option value="send_to_all_operators"<?php if($this->route['route_action'] == 'send_to_all_operators') { ?> selected="selected"<?php } ?>><?php echo JText::_('SEND_TO_ALL_OPERATORS'); ?></option>
		    <option value="send_to_selected_operators"<?php if($this->route['route_action'] == 'send_to_selected_operators') { ?> selected="selected"<?php } ?>><?php echo JText::_('SEND_TO_SELECTED_OPERATORS'); ?></option>
		    <option value="send_to_offline"<?php if($this->route['route_action'] == 'send_to_offline') { ?> selected="selected"<?php } ?>><?php echo JText::_('SEND_TO_OFFLINE_MESSAGE'); ?></option>
		</select><select style="margin-left: 20px;" name="ring_type">
		    <option value="default"<?php if($this->route['route_source_data']->ring_type == 'default') { ?> selected="selected"<?php } ?>>- <?php echo JText::_('DEFAULT_RING_ORDER'); ?> -</option>
		    <option value="ring_at_same_time"<?php if($this->route['route_source_data']->ring_type == 'ring_at_same_time') { ?> selected="selected"<?php } ?>><?php echo JText::_('RING_OPERATORS_AT_SAME_TIME'); ?></option>
		    <option value="ring_in_order"<?php if($this->route['route_source_data']->ring_type == 'ring_in_order') { ?> selected="selected"<?php } ?>><?php echo JText::_('RING_OPERATORS_IN_ORDER'); ?></option>
		</select>
	    </td>
	</tr>
    </table>
	...
	<tr>
		...
		<select id="available_operators" multiple="multiple" size="10" style="width: 230px; height: 130px;" disabled="disabled">
		    <?php
		    for($a = 0; $a < count($this->operators); $a++)
		    {
			if(!in_array($this->operators[$a]['operator_id'], $this->route['route_source_data']->selected_operators))
			{
		    ?>
		...
		<select id="selected_operators" name="selected_operators[]" multiple="multiple" size="10" style="width: 230px; height: 130px;" disabled="disabled">
		    <?php
		    for($a = 0; $a < count($this->route['route_source_data']->selected_operators); $a++)
		    {
		    ?>
		    <option value="<?php echo $this->route['route_source_data']->selected_operators[$a]; ?>"><?php echo $this->operatoradmin->operatorKeyLabel($this->route['route_source_data']->selected_operators[$a]); ?></option>
		    <?php
		    }
		    ?>
		</select>
...
<script type="text/javascript">
    ...
    <br />
    <input type="hidden" name="route_id" value="<?php echo $this->route['route_id']; ?>" />
    <input type="hidden" name="task" value="update_route" />
