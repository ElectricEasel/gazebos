...
class JFormFieldProductOptions extends JFormField
{
	...
	protected function getInput()
	{
		...
		$html[] = '<p>Click into the field and select from the options in the dropdown. To remove an option, click the x next to the option text.</p>';

		foreach ($options as $id => $keys)
		{
			$sel = isset($selected[$id]) ? $selected[$id] : null;
	...
	protected function getOptions()
	{
		...
		$option_type = (int) $this->element['option_type'];
		...
		$q
			...
			->where('b.option_type = ' . $option_type);
